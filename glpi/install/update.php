...
if (empty($_POST["continuer"]) && empty($_POST["from_update"])) {
    if (empty($from_install) && !isset($_POST["from_update"])) {
        ...
        echo "<a class='btn btn-primary' href='../index.php'>
            " . __('Go back to CLOUD4DEV') . "
         </a></p>";
        echo "</div>";
    } else {
        echo "<div class='center'>";
        echo "<h3 class='my-4'><span class='migred p-2'>" . sprintf(__('Caution! You will update the C4D database named: %s'), $DB->dbdefault) . "</h3>";
